...
export async function fetchNews({ force = false } = {}) {
  ...
  if (!apiKey) {
    throw new Error('Missing GNews API key. Add VITE_GNEWS_API_KEY to your .env file.')
  }
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error('GNews rejected the API key. Check VITE_GNEWS_API_KEY and retry.')
    }
  ...
  if (data.errors) {
    throw new Error('GNews returned an API error. Please check the key or retry later.')
  }
